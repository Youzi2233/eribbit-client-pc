...
export default {
  ...
  actions: {
    ...
    batchDeleteCart(ctx, isClear) {
      return new Promise((resolve, reject) => {
        if (ctx.rootState.user.profile.token) {
          // 已登录
          const ids = ctx.getters[isClear ? 'unvalidList' : 'selectedList'].map(item => item.skuId);
          deleteCart(ids)
            .then(() => {
              return findCart();
            })
  ...
  mutations: {
    ...
    updateCart(state, goods) {
      ...
      // goods 商品对象的字段不固定，对象中就哪些字段就改哪些字段，字段的值合理才改
      // goods 商品对象，必须有skuId
