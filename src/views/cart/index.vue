        ...
        <table>
          ...
          <tbody>
            ...
            <tr v-for="goods in $store.getters['cart/validList']" :key="goods.skuId">
              ...
              <td>
                <div class="goods">
                  <router-link :to="`/product/${goods.id}`"><img :src="goods.picture" alt="" /></router-link>
                  <div>
...
export default {
  ...
  setup() {
    ...
    const batchDeleteCart = isClear => {
      if (store.getters['cart/unvalidList'].length === 0 && isClear) {
        Message({ text: '购物车暂无失效商品' });
        return;
      }
      Confirm({ text: `亲，您是否确认删除${isClear ? '失效' : '已选中'}的商品` }).then(() => {
    ...
    return { checkout, checkOne, checkAll, deleteCart, batchDeleteCart, updateCount, updateCartSku };
  },
};
</script>
